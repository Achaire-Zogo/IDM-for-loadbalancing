...
\section{Implémentation Java du validateur (LoadBalancerValidator.java)}

Les contraintes OCL sont implémentées dans le validateur généré par Xtext. Chaque méthode annotée \texttt{@Check} correspond à une règle de gestion :

\begin{lstlisting}[language=Java, caption={LoadBalancerValidator.java -- Méthodes de validation}]
...
public class LoadBalancerValidator
    extends AbstractLoadBalancerValidator {
  ...
  public void checkUniqueServerNames(Cluster cluster) {
    ...
    for (Server s : cluster.getServers()) {
      if (!names.add(s.getName())) {
        error("Nom de serveur duplique: " + s.getName(),
  ...
  public void checkValidPort(Server server) {
    if (server.getPort() < 1 || server.getPort() > 65535) {
      error("Le port doit etre entre 1 et 65535",
  ...
  public void checkValidListenerPort(Listener listener) {
    if (listener.getPort() < 1 || listener.getPort() > 65535) {
      error("Le port du listener doit etre entre 1 et 65535",
  ...
  public void checkMinMaxInstances(ScalingRule rule) {
    if (rule.getMinInstances() > rule.getMaxInstances()) {
      error("minInstances doit etre <= maxInstances",
  ...
  public void checkUniqueListenerPorts(LoadBalancerSystem system) {
    ...
    for (Listener l : system.getListeners()) {
      if (!ports.add(l.getPort())) {
        error("Port de listener duplique: " + l.getPort(),
          LoadBalancerPackage.Literals
            .LOAD_BALANCER_SYSTEM__LISTENERS);
      }
    }
  }
  ...
  public void checkWeightedRequiresWeights(Cluster cluster) {
    if (cluster.getLoadBalancer().getAlgorithm()
        == Algorithm.WEIGHTED_ROUND_ROBIN) {
      for (Server s : cluster.getServers()) {
        if (s.getWeight() < 1) {
          error("Algorithme WEIGHTED_ROUND_ROBIN requiert "
            + "un poids >= 1 pour chaque serveur",
            LoadBalancerPackage.Literals.CLUSTER__SERVERS);
        }
      }
    }
  ...
  public void checkCookieRequiresName(SessionPersistence sp) {
    if (sp.getType() == PersistenceType.COOKIE) {
      if (sp.getCookieName() == null
          || sp.getCookieName().isEmpty()) {
        error("Le type COOKIE requiert un cookieName",
          LoadBalancerPackage.Literals
            .SESSION_PERSISTENCE__COOKIE_NAME);
      }
    }
  }
  ...
  public void checkAtLeastOneListener(LoadBalancerSystem system) {
    if (system.getListeners().isEmpty()) {
      error("Le systeme doit avoir au moins un listener",
        LoadBalancerPackage.Literals
          .LOAD_BALANCER_SYSTEM__LISTENERS);
    }
  }
  ...
  public void checkAtLeastOneEnabledServer(Cluster cluster) {
    ...
    for (Server s : cluster.getServers()) {
      if (s.isEnabled()) { hasEnabled = true; break; }
    }
    if (!hasEnabled) {
...
\subsection{Correspondance OCL $\leftrightarrow$ Java}

\begin{table}[H]
\centering
\caption{Correspondance entre contraintes OCL et méthodes Java}
\begin{tabular}{|c|l|l|}
\hline
\textbf{Règle} & \textbf{Invariant OCL} & \textbf{Méthode Java} \\
\hline
RG01 & \texttt{uniqueServerNames} & \texttt{checkUniqueServerNames()} \\
\hline
RG02 & \texttt{validPort} & \texttt{checkValidPort()} \\
\hline
RG03 & \texttt{positiveWeight} & \texttt{checkPositiveWeight()} \\
\hline
RG04 & \texttt{nonEmptyCluster} & \texttt{checkNonEmptyCluster()} \\
\hline
RG05 & \texttt{intervalGreaterThanTimeout} & \texttt{checkHealthCheckCoherence()} \\
\hline
RG06 & \texttt{scaleUpGreaterThanDown} & \texttt{checkScalingThresholds()} \\
\hline
RG07 & \texttt{minLessOrEqualMax} & \texttt{checkMinMaxInstances()} \\
\hline
RG08 & \texttt{uniqueListenerPorts} & \texttt{checkUniqueListenerPorts()} \\
\hline
RG09 & \texttt{weightedRequiresWeights} & \texttt{checkWeightedRequiresWeights()} \\
\hline
RG10 & \texttt{cookieRequiresName} & \texttt{checkCookieRequiresName()} \\
\hline
RG11 & \texttt{atLeastOneListener} & \texttt{checkAtLeastOneListener()} \\
\hline
RG12 & \texttt{atLeastOneEnabledServer} & \texttt{checkAtLeastOneEnabledServer()} \\
\hline
\end{tabular}
\end{table}
...
\section{Métamodèle PSM Nginx (nginx.ecore)}

Le métamodèle PSM Nginx représente la structure d'une configuration Nginx. Il est la cible de la transformation ATL (T1) et la source du template Acceleo (T3).

\begin{lstlisting}[language=XML, caption={Métamodèle PSM Nginx (nginx.ecore)}]
<?xml version="1.0" encoding="UTF-8"?>
<ecore:EPackage xmi:version="2.0"
    xmlns:xmi="http://www.omg.org/XMI"
    xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    xmlns:ecore="http://www.eclipse.org/emf/2002/Ecore"
    name="nginx"
    nsURI="http://www.example.org/nginx"
    nsPrefix="nginx">

  <!-- Racine : configuration globale Nginx -->
  <eClassifiers xsi:type="ecore:EClass" name="NginxConfig">
    <eStructuralFeatures xsi:type="ecore:EAttribute"
        name="workerProcesses" eType="ecore:EDataType
        http://www.eclipse.org/emf/2002/Ecore#//EString"
        defaultValueLiteral="auto"/>
    <eStructuralFeatures xsi:type="ecore:EAttribute"
        name="workerConnections" eType="ecore:EDataType
        http://www.eclipse.org/emf/2002/Ecore#//EInt"
        defaultValueLiteral="1024"/>
    <eStructuralFeatures xsi:type="ecore:EReference"
        name="upstreams" upperBound="-1"
        eType="#//Upstream" containment="true"/>
    <eStructuralFeatures xsi:type="ecore:EReference"
        name="servers" upperBound="-1"
        eType="#//ServerBlock" containment="true"/>
  </eClassifiers>

  <!-- Bloc upstream (groupe de backends) -->
  <eClassifiers xsi:type="ecore:EClass" name="Upstream">
    <eStructuralFeatures xsi:type="ecore:EAttribute"
        name="name" eType="ecore:EDataType
        http://www.eclipse.org/emf/2002/Ecore#//EString"/>
    <eStructuralFeatures xsi:type="ecore:EAttribute"
        name="algorithm" eType="ecore:EDataType
        http://www.eclipse.org/emf/2002/Ecore#//EString"/>
    <eStructuralFeatures xsi:type="ecore:EReference"
        name="servers" upperBound="-1"
        eType="#//UpstreamServer" containment="true"/>
  </eClassifiers>

  <!-- Serveur dans un upstream -->
  <eClassifiers xsi:type="ecore:EClass" name="UpstreamServer">
    <eStructuralFeatures xsi:type="ecore:EAttribute"
        name="address" eType="ecore:EDataType
        http://www.eclipse.org/emf/2002/Ecore#//EString"/>
    <eStructuralFeatures xsi:type="ecore:EAttribute"
        name="weight" eType="ecore:EDataType
        http://www.eclipse.org/emf/2002/Ecore#//EInt"
        defaultValueLiteral="1"/>
    <eStructuralFeatures xsi:type="ecore:EAttribute"
        name="maxFails" eType="ecore:EDataType
        http://www.eclipse.org/emf/2002/Ecore#//EInt"
        defaultValueLiteral="3"/>
    <eStructuralFeatures xsi:type="ecore:EAttribute"
        name="failTimeout" eType="ecore:EDataType
        http://www.eclipse.org/emf/2002/Ecore#//EString"
        defaultValueLiteral="30s"/>
    <eStructuralFeatures xsi:type="ecore:EAttribute"
        name="backup" eType="ecore:EDataType
        http://www.eclipse.org/emf/2002/Ecore#//EBoolean"
        defaultValueLiteral="false"/>
  </eClassifiers>

  <!-- Bloc server (point d'entree) -->
  <eClassifiers xsi:type="ecore:EClass" name="ServerBlock">
    <eStructuralFeatures xsi:type="ecore:EAttribute"
        name="listenPort" eType="ecore:EDataType
        http://www.eclipse.org/emf/2002/Ecore#//EInt"/>
    <eStructuralFeatures xsi:type="ecore:EAttribute"
        name="serverName" eType="ecore:EDataType
        http://www.eclipse.org/emf/2002/Ecore#//EString"
        defaultValueLiteral="_"/>
    <eStructuralFeatures xsi:type="ecore:EAttribute"
        name="proxyPass" eType="ecore:EDataType
        http://www.eclipse.org/emf/2002/Ecore#//EString"/>
    <eStructuralFeatures xsi:type="ecore:EReference"
        name="locations" upperBound="-1"
  ...
  <eClassifiers xsi:type="ecore:EClass" name="Location">
    <eStructuralFeatures xsi:type="ecore:EAttribute"
        name="path" eType="ecore:EDataType
        http://www.eclipse.org/emf/2002/Ecore#//EString"
        defaultValueLiteral="/"/>
    <eStructuralFeatures xsi:type="ecore:EAttribute"
        name="proxyPass" eType="ecore:EDataType
        http://www.eclipse.org/emf/2002/Ecore#//EString"/>
    <eStructuralFeatures xsi:type="ecore:EAttribute"
        name="proxySetHeaders" upperBound="-1"
...
\subsection{Diagramme du métamodèle PSM Nginx}

\begin{figure}[H]
\centering
\begin{tikzpicture}[
  class/.style={rectangle, draw, fill=white, minimum width=3.8cm, minimum height=1.4cm, align=center, font=\small},
  arrow/.style={-{Stealth}, thick},
  comp/.style={-{Diamond[open]}, thick}
]
  \node[class] (config) at (0,0) {\textbf{NginxConfig}\\workerProcesses, workerConnections};
  \node[class] (upstream) at (-4,-3) {\textbf{Upstream}\\name, algorithm};
  \node[class] (server) at (4,-3) {\textbf{ServerBlock}\\listenPort, serverName, proxyPass};
  \node[class] (us) at (-4,-6) {\textbf{UpstreamServer}\\address, weight, maxFails, backup};
  \node[class] (loc) at (4,-6) {\textbf{Location}\\path, proxyPass, proxySetHeaders};

  \draw[comp] (config) -- node[left, font=\scriptsize] {0..*} (upstream);
  \draw[comp] (config) -- node[right, font=\scriptsize] {0..*} (server);
  \draw[comp] (upstream) -- node[left, font=\scriptsize] {0..*} (us);
  \draw[comp] (server) -- node[right, font=\scriptsize] {0..*} (loc);
\end{tikzpicture}
\caption{Diagramme de classes du métamodèle PSM Nginx}
\end{figure}
...
\begin{enumerate}
  ...
  \item Exécuter : Clic droit Dsl.xtext $\rightarrow$ \texttt{Run As $\rightarrow$ Generate Xtext Artifacts}
...
\begin{enumerate}
  \item ajouter les constraints OCL dans le fichier ecore (voir chapitre 7)
  \item Ouvrir le fichier \texttt{LoadBalancerValidator.java} généré par Xtext
  \item Ajouter les méthodes de validation (voir chapitre 7)
...
\subsection{Création du projet Acceleo}

\begin{enumerate}
  \item \texttt{File $\rightarrow$ New $\rightarrow$ Project $\rightarrow$ Acceleo $\rightarrow$ Acceleo Project}
  \item Nommer le projet : \texttt{org.loadbalancer.generators}
  \item Sélectionner le métamodèle source :
    \begin{itemize}
      \item Pour T3 (nginx.conf) : \texttt{http://www.example.org/nginx}
      \item Pour T5 (docker-compose) : \texttt{http://www.example.org/loadbalancer}
    \end{itemize}
  \item Cliquer sur \texttt{Finish}
\end{enumerate}

La structure du projet générée est la suivante :

\begin{lstlisting}
org.loadbalancer.generators/
  src/
    org.loadbalancer.generators/
      files/
        generateNginx.mtl          # Template nginx.conf
        generateDockerCompose.mtl  # Template docker-compose.yml
  META-INF/
    MANIFEST.MF
  build.properties
\end{lstlisting}

\subsection{Création des templates .mtl}

\paragraph{Template T3 : generateNginx.mtl}

Créer le fichier \texttt{src/org.loadbalancer.generators/files/generateNginx.mtl} avec le contenu suivant :

\begin{lstlisting}[caption={Template Acceleo generateNginx.mtl}]
[comment encoding = UTF-8 /]
[module generateNginx('http://www.example.org/nginx')]
...
[for (server : UpstreamServer | upstream.servers)]
        server [server.address/] weight=[server.weight/]
            ...
            fail_timeout=[server.failTimeout/][if (server.backup)]
            backup[/if];
[/for]
    }
...
\paragraph{Template T5 : generateDockerCompose.mtl}

Créer le fichier \texttt{src/org.loadbalancer.generators/files/generateDockerCompose.mtl} avec le contenu suivant :

\begin{lstlisting}[caption={Template Acceleo generateDockerCompose.mtl}]
[comment encoding = UTF-8 /]
[module generateDockerCompose(
  'http://www.example.org/loadbalancer')]

[template public generateCompose(
...
[for (server : Server | cluster.servers)]
  [server.name/]:
    build: ./servers/[if (cluster.name = 'WebFrontend')]
web[else]api[/if]
    environment:
...
[if (cluster.healthCheck <> null)]
      test: ['['"CMD"'/]', '['"curl"'/]', '['"
        -f"'/]', '['"[cluster.healthCheck.protocol
        .toString().toLower()/]://localhost:
        [server.port/][cluster.healthCheck.path/]
        "'/]']
...
\subsection{Configuration du Launch Configuration}

Pour exécuter les templates Acceleo, il faut créer une configuration de lancement :

\begin{enumerate}
  \item \texttt{Run $\rightarrow$ Run Configurations...}
  \item Clic droit sur \texttt{Acceleo Application} $\rightarrow$ \texttt{New Configuration}
  \item Configurer les champs suivants :
\end{enumerate}

\begin{table}[H]
\centering
\caption{Configuration du Launch Configuration Acceleo}
\begin{tabular}{|p{4cm}|p{9cm}|}
\hline
\textbf{Champ} & \textbf{Valeur} \\
\hline
\texttt{Project} & \texttt{org.loadbalancer.generators} \\
\hline
\texttt{Main class} & \texttt{org.loadbalancer.generators.files.GenerateNginx} \\
\hline
\texttt{Model} & Chemin vers le modèle source, par exemple : \texttt{/org.loadbalancer.examples/models/ecommerce.nginx} (pour T3) ou \texttt{/org.loadbalancer.examples/models/ecommerce.lb} (pour T5) \\
\hline
\texttt{Target} & Répertoire de sortie des fichiers générés, par exemple : \texttt{/org.loadbalancer.examples/generated/} \\
\hline
\end{tabular}
\end{table}
...
\begin{enumerate}
  \setcounter{enumi}{3}
  \item Cliquer sur \texttt{Apply} puis \texttt{Run}
  \item Les fichiers générés apparaissent dans le répertoire cible :
    \begin{itemize}
      \item \texttt{generated/nginx.conf} -- configuration Nginx
      \item \texttt{generated/docker-compose.yml} -- orchestration Docker
    \end{itemize}
\end{enumerate}

\subsection{Vérification des fichiers générés}

Après exécution, vérifier que les fichiers générés sont corrects :

\begin{lstlisting}[language=bash, caption={Vérification des fichiers générés}]
# Verifier la syntaxe nginx
nginx -t -c $(pwd)/generated/nginx.conf

# Verifier la syntaxe docker-compose
docker compose -f generated/docker-compose.yml config

# Si les deux commandes passent sans erreur,
# les fichiers generes sont valides
\end{lstlisting}
